...
6. **Message-type router**:
   ...
   - audio → base64 → binary → speech-to-text transcription.
...
9. **AI Agent** runs with:
   ...
   - the **RAG retrieval tool** (Supabase vector store + embeddings),
...
3. **Delete previous chunks** for that `file_id` in the vector store (idempotent
   re-ingestion).
4. **Download + extract text** from the file.
5. **Split by separator** (custom code) then the **recursive character text
   splitter** chunks the content.
6. **Embeddings** are generated and the chunks are **upserted into the Supabase
   vector store** with metadata (`file_id`, `file_name`, `source`).
...
1. **Update the lead** — add the `Atendimento Humano` tag.
2. **Notify the team** — send a WhatsApp message with the reason and full context
   to the operations number.

On the next inbound message the main workflow sees the tag and stops answering
automatically.
...
| RAG ingestion | Main orchestrator | Shared Supabase `documents` vector table |
| Main orchestrator | Main orchestrator | Redis buffer + PostgreSQL chat memory (per-contact state) |
...
flowchart TD
    ...
    Supabase[("Supabase<br/>Vector Store / pgvector")]
    Postgres[("PostgreSQL<br/>memory + leads")]
    Redis[("Redis")]
    GoogleCalendar[("Google Calendar")]
    EmbedProvider["Embeddings provider"]
    LLM["LLM provider<br/>(chat + audio)"]
